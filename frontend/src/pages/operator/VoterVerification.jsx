...
const VoterVerification = () => {
    ...
    const [scanQuality, setScanQuality] = useState(null); // null | 'Poor' | 'Good' | 'Excellent'
    const [hardwareStatus, setHardwareStatus] = useState('Offline'); // Offline | Connected | Ready
    const [duplicateInfo, setDuplicateInfo] = useState(null);

    // Simulate Hardware Connection on component mount
    useEffect(() => {
        const timer1 = setTimeout(() => setHardwareStatus('Connected'), 1000);
        const timer2 = setTimeout(() => setHardwareStatus('Ready'), 2500);
    ...
    const handleStartScan = async () => {
        if (hardwareStatus !== 'Ready') {
            toast.error('Hardware is not ready yet.');
            return;
        }
        setScanState('scanning');
        setScanQuality(null);

        // Simulate scan pulses
        const qualityInterval = setInterval(() => {
            const q = Math.random();
            setScanQuality(q < 0.3 ? 'Poor' : q < 0.7 ? 'Good' : 'Excellent');
        }, 500);
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-6 sm:p-8 shadow-sm">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h2 className="text-lg sm:text-xl font-bold text-slate-800">{t('step_3_title')}</h2>
                                </div>
                                {/* Hardware Status Indicator */}
                                <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 border border-slate-200 rounded-full">
                                    <div className={`w-2 h-2 rounded-full ${hardwareStatus === 'Connected' ? 'bg-amber-400' :
                                        hardwareStatus === 'Ready' ? 'bg-green-500' : 'bg-slate-400'
                                        }`} />
                                    <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">{hardwareStatus}</span>
                                </div>
                            </div>
                            ...
                            {/* Scanner Area */}
                            <div className={`
                                relative rounded-xl h-48 flex flex-col items-center justify-center transition-all duration-500 overflow-hidden
                            ...
                            `}>
                                ...
                                {scanState === 'scanning' && (
                                    <div className="absolute inset-0 z-0 flex flex-col items-center justify-center opacity-20">
                                        <div className="w-full h-1 bg-blue-400 animate-[scan_2s_ease-in-out_infinite] shadow-[0_0_15px_rgba(96,165,250,0.8)]" />
                                    </div>
                                )}
                                ...
                                <div className="z-10 flex flex-col items-center">
                                    {scanState === 'idle' && (
                                        <>
                                            <div className="w-16 h-16 mb-2 rounded-full border-2 border-slate-600 flex items-center justify-center opacity-50">
                                                <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4" /></svg>
                                            </div>
                                            <p className="text-sm font-medium">{t('scan_waiting')}</p>
                                        </>
                                    )}
                                    {scanState === 'scanning' && (
                                        <>
                                            <div className="relative mb-2">
                                                <div className="w-16 h-16 rounded-full border-2 border-blue-500/30 flex items-center justify-center animate-pulse">
                                                    <svg className="w-8 h-8 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4" /></svg>
                                                </div>
                                            </div>
                                            <p className="text-sm font-bold text-blue-400">{t('scan_scanning')}</p>
                                        </>
                                    )}
                                    {scanState === 'success' && (
                                        <>
                                    ...
                                    {scanQuality && (
                                        <div className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-slate-900/50 px-2 py-1 rounded backdrop-blur-sm">
                                            <span className="text-[9px] text-slate-400 uppercase tracking-wider">Quality:</span>
                                            <span className={`text-xs font-bold ${scanQuality === 'Excellent' ? 'text-green-400' :
                                                scanQuality === 'Good' ? 'text-blue-400' : 'text-red-400'
                                                }`}>{scanQuality}</span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="flex gap-3 mt-4">
                                {scanState === 'idle' && (
                                    <>
                                        <button
                                            ...
                                            disabled={hardwareStatus !== 'Ready'}
                                            className="px-5 py-2.5 bg-janmat-blue text-white text-sm font-bold rounded-lg hover:bg-janmat-hover transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {hardwareStatus === 'Ready' ? t('btn_start_scan') : 'Initializing Device...'}
                                        </button>
                                        <button
                                ...
                                {scanState === 'success' && (
                                    <button
                                        ...
                                        className="w-full py-3 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-700 transition-all shadow-sm"
                                    >
                                        {t('btn_mark_voted')}
                                    </button>
                                )}
